...
private:
  ...
  TimeDelayKalmanFilter ekf_;  //!< @brief  extended kalman filter instance.实例

  /* parameters */
  bool show_debug_info_;   //!< @brief 显示debug信息`
  ...
  geometry_msgs::PoseStamped current_ekf_pose_;                     //!< @brief current estimated pose 空间6dof姿态
  geometry_msgs::TwistStamped current_ekf_twist_;                   //!< @brief current estimated twist 线速度角速度
  boost::array<double, 36ul> current_pose_covariance_;           //!< @brief协方差矩阵
  boost::array<double, 36ul> current_twist_covariance_;   

  /**
   * @brief computes update & prediction of EKF for each ekf_dt_[s] time 计算EKF
   */
  ...
  /**
   * @brief set current EKF estimation result to current_ekf_pose_ & current_ekf_twist_ 这是啥
   */
